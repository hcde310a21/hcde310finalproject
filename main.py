...
def get_info(recentdict, recsdict):
    ...
    for game in recentdict["responses"]["games"]: # call must pass in recentdict with get_recent_games
        combinedurl = steamurl + "ISteamNews/GetNewsForApp/v2/?appid=" + str(game["appid"]) # can add max length to content or count for # of posts
        safedata = safe_get(combinedurl)
    ...
    for gameid in recsdict:
        combinedurl = steamurl + "ISteamNews/GetNewsForApp/v2/?appid=" + str(gameid) # can add max length to content or count for # of posts
        safedata = safe_get(combinedurl)
        news = json.load(safedata)
